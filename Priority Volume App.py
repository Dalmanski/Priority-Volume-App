...
class VolumeMeter(QWidget):
    ...
    def paintEvent(self, event):
        ...
        l = self.level
        if l <= 0.33:
            fill_color = QColor(50, 220, 90)  # green
        elif l <= 0.66:
            fill_color = QColor(255, 165, 40)  # orange
        else:
            fill_color = QColor(220, 60, 60)   # red
...
class AppRow(QFrame):
    ...
    def mousePressEvent(self, event):
        try:
            pos = event.pos()
            if self.slider.geometry().contains(pos):
...
class VolumeController(QMainWindow):
    ...
    def load_settings(self):
        default = {"priority_percent": 100, "background_percent": 20, "auto_priority_enabled": False, "auto_100_enabled": False, "run_at_startup": False}
        try:
